...
// Get gold rates (using multiple sources for reliability)
$goldPriceUSD = null;

// Try primary gold API
try {
    ...
    if ($goldData && isset($goldData['data']['rates']['USD'])) {
        $goldPriceUSD = $goldData['data']['rates']['USD']; // Direct USD price per ounce
    }
} catch (Exception $e) {
    // Try alternative gold API
    try {
        $altGoldUrl = 'https://api.coinbase.com/v2/exchange-rates?currency=XAU';
        $altGoldResponse = @file_get_contents($altGoldUrl);
        $altGoldData = json_decode($altGoldResponse, true);
        
        if ($altGoldData && isset($altGoldData['data']['rates']['USD'])) {
            $goldPriceUSD = 1 / floatval($altGoldData['data']['rates']['USD']);
        }
    } catch (Exception $e2) {
        // Use fallback rates
    }
